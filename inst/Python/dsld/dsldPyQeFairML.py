...
### qeFairKNN-------------------------------------------------------------------
def dsldPyQeFairKNN(data, yName, sNames, deweightPars = R_NULL, yesYVal = R_NULL, k = 25, scaleX = True):
    r_data = dsld_Rpy2_IsRDataframe(data)
    yName = robjects.StrVector([yName])
    sNames = robjects.StrVector([sNames])

    if deweightPars != R_NULL:
...
## dsldQeFairRF-------------------------------------------------------------------
def dsldPyQeFairRF(data, yName, sNames, deweightPars = R_NULL, nTree = 500, minNodeSize = 10, mtry = None, yesYVal = R_NULL):    
...
def dsldPyQeFairRidgeLin(data, yName, sNames, deweightPars = R_NULL):
    r_data = dsld_Rpy2_IsRDataframe(data)
    yName = robjects.StrVector([yName])
    sNames = robjects.StrVector([sNames])
...
def dsldPyQeFairRidgeLog(data, yName, sNames, deweightPars = R_NULL, yesYVal = R_NULL):
    r_data = dsld_Rpy2_IsRDataframe(data)
    yName = robjects.StrVector([yName])
    sNames = robjects.StrVector([sNames])
